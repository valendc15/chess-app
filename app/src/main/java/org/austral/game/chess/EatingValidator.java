...
public class EatingValidator implements Validator {
    ...
    public boolean validate(Movement movement, Board board, Player player) {
        return board.getSquares(movement.getTo()).hasPiece() && board.getPiecefromPosition(movement.getFrom()).getColor() != board.getPiecefromPosition(movement.getTo()).getColor();
    }
